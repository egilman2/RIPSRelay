...
class LightningModule(pl.LightningModule):
    ...
    def get_loss(self, smiles_emb, measures):
        ...
        return self.loss(z_pred, measures), z_pred, measures, smiles_emb
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, pred, actual, embs = self.get_loss(loss_input, targets)
    ...
    def validation_epoch_end(self, outputs):
        ...
        for dataset_idx, batch_outputs in enumerate(outputs):
            ...
            embs = torch.cat([x['embs'] for x in batch_outputs])
            embs_cpu = embs.detach().cpu().numpy()
            if dataset == 'test' and self.current_epoch % 5 == 0:                 
                predac = pd.DataFrame()
                predac['preds'] = preds_cpu
                predac['actual'] = actuals_cpu
                embsetp = pd.DataFrame(embs_cpu)
                if not os.path.exists(os.path.join(
                        self.hparams.results_dir, "preds")):
                    os.makedirs(os.path.join(
                        self.hparams.results_dir, "preds"))
                if not os.path.exists(
                        os.path.join(self.hparams.results_dir, "embies")):
                    os.makedirs(os.path.join(
                        self.hparams.results_dir, "embies"))
                predac.to_csv(os.path.join(
                    self.hparams.results_dir, "preds/preds_epoch_" +
                                              str(self.current_epoch) + ".csv"))
                embsetp.to_csv(os.path.join(
                    self.hparams.results_dir, "embies/embies_epoch_" +
                                              str(self.current_epoch) + ".csv"))
